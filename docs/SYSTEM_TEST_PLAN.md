...
|---|-------|------|-------|--------|--------|---------|------------|
| 0.1 | Prerequisites | `build-status` clean | A | PASS | PASS | PASS | PASS |
| 0.2 | Prerequisites | `ltvm doctor` no orphans / stale TAP | A | PASS | PASS | PASS | PASS |
| 0.3 | Prerequisites | Host bridge + DNS reachable from VM | A | PASS | PASS | PASS | PASS |
| 1.1 | VM Creation | Default: 2 vCPU, 2 GiB, 1 MDT, 2 OST, 500 MiB | A | PASS | PASS | PASS | PASS |
| 1.2 | VM Creation | `ensure` on running VM: exit 0, no second process | A | PASS | PASS | PASS | PASS |
| 1.3 | VM Creation | `--disk-size` override reflected in `lsblk` | A | PASS | PASS | PASS | PASS |
| 1.4 | VM Creation | `list --json` all expected fields non-null | A | PASS | PASS | PASS | PASS |
| 1.5 | VM Creation | Overlay is qcow2 with backing file (not flat copy) | A | PASS | PASS | PASS | PASS |
| 1.6 | VM Creation | Root fs resized to 8 GiB on first boot | A | PASS | PASS | PASS | PASS |
...
| 2.5 | Disk Topology | Client only (0/0) — no `/dev/vdb` | A | PASS | PASS | PASS | PASS |
...
| 6.1 | Crash/kdump | NMI triggers panic + reboot | A | PASS | PASS | PASS | PASS |
| 6.2 | Crash/kdump | vmcore present and > 1 MiB in `/var/crash/` | A | PASS | PASS | PASS | FAIL* |
| 6.3 | Crash/kdump | `crash-tool` / `lustre_triage.py` parses vmcore | A | PASS | PASS | PASS | FAIL* |
| 6.4 | Crash/kdump | `kdump.service` active after reboot | A | PASS | PASS | PASS | PASS |
| 6.5 | Crash/kdump | vmlinux build-id matches running kernel | A | PASS | PASS | PASS | PASS |
| 6.6 | Crash/kdump | `crash-collect --trigger` (sysrq path) works | A | PASS | PASS | PASS | FAIL* |
...
| 8.1 | Client Build | Build with `--disable-server` (no ldiskfs/obdfilter) | C | N/A | PASS | PASS | PASS |
| 8.2 | Client Build | `lsmod` shows client modules, not server modules | C | N/A | PASS | PASS | PASS |
| 8.3 | Client Build | Client VM mounts a rocky9 server's Lustre over TCP | C | N/A | PASS | PASS | PASS |
| 8.4 | Client Build | `lfs df` on client shows server OSTs | C | N/A | PASS | PASS | PASS |
| 8.5 | Client Build | Write + read-back + checksum from client | C | N/A | PASS | PASS | PASS |
| 9.1 | Snapshot | Snapshot, corrupt state, restore, Lustre remounts | S | | N/A | N/A | N/A |
| 9.2 | Snapshot | Restore to non-existent tag exits non-zero | A | | PASS | PASS | PASS |
| 9.3 | Snapshot | Snapshot while running: VM stopped + restarted | A | | PASS | PASS | PASS |
...
4. **ubuntu2404** — client-only, all applicable phases ✓ complete (experimental)

`*` = kexec does not work on QEMU microvm with Ubuntu 6.8 kernel.
NMI triggers panic + reboot but the crash kernel is not invoked,
so no vmcore is captured. kdump-tools loads and reports "ready"
but kexec_load fails silently on the microvm machine type.
